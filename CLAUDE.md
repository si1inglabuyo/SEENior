...
- **`users`** — `id`, `username`, `password_hash` (nullable — null for Google-only accounts), `role` (`family_contact` / `barangay_responder`), `full_name`, `phone`, `email` (nullable/unique — family sign-in identity; barangay responders log in by `username` instead and leave this null), `google_sub` (nullable/unique — Google's stable per-account ID), `barangay` (scopes a barangay responder's dashboard queries; unused for family contacts), `is_active`, `created_at`, `deleted_at` / `deletion_reason` / `deletion_note` (nullable — soft account deletion, see §11a).
- **`seniors`** — `id`, `sync_id` (anonymous UUID used for all cloud-facing references, per §11), `first_name`, `last_name`, `age`, `gender`, `barangay`, `address` (joined PSGC-validated string), `mobile_number`, `invite_code` (nullable, 6-digit, time-limited), `invite_code_expires_at`, `created_at`, `deleted_at` / `deletion_reason` / `deletion_note` (nullable — soft account deletion, see §11a).
...
- **Crash reports leave the device (Firebase Crashlytics, added 2026-09-23).** A stack trace, the device model, the OS/app version, a Crashlytics install id, and three device-capability keys (`has_step_counter`, `battery_exempt`, `os_build`). This does **not** breach the raw-sensor-data boundary above: Crashlytics is handed nothing from the local database, it does not read logcat, and `setUserId` is deliberately never called — a report says *what broke on which model of phone*, never *who was using it*. Added because a crash on a tester's handset was otherwise invisible: the foreground-service crash fixed the same day had been live since the 09-18 rollout and was found only because a laptop happened to be plugged in five days later. Keep it that way — do not attach a senior's name, number, address, location, sync_id or any sensor reading to a crash report.

### §11a — Account deletion is soft, on both sides

Both apps have a **Delete account** flow (senior: Profile → Account; family: Profile, under Log Out). Each requires a **reason** — a stable code from a picker (`switching_phone`, `privacy`, …), never the translated label — plus an optional free-text note, stored on the row as `deletion_reason` / `deletion_note`.

- **Family account** (`POST /auth/me/delete`, JWT-authenticated): the `users` row is kept for audit with `deleted_at` set; `is_active` goes False (so `login()` / `get_current_user()` already reject it); every `contacts` pairing is soft-unlinked (`unlinked_by = family`); `device_tokens` rows are deleted; and `username` / `email` / `google_sub` are tombstoned with a `+del<id>.<ts>` tag (front-truncated to the column limit — `username` is only 64 chars) so those unique slots free up for a fresh sign-up — no constraint change, deliberately not a partial index.
- **Senior account** (`POST /seniors/{sync_id}/delete`, no auth — the `sync_id` is the credential): the `seniors` row is kept with `deleted_at` set; its `contacts` are soft-unlinked (`unlinked_by = senior`); `push_token` / `last_nudge_at` / `invite_code` are cleared. The phone then **wipes its entire local SQLite database** (`clearAllTables()`), stops the foreground monitoring service, cancels armed escalation alarms, and returns to the pre-onboarding welcome screen. Re-onboarding mints a brand-new `sync_id`. Known limitation: the cloud call is best-effort with no post-wipe retry, so an offline delete leaves the name/address-only cloud row until it is pruned by hand.
- **Every user-facing query must exclude `deleted_at IS NOT NULL`.** Done in `contacts.py` (invite-code lookup), `escalation.py` (sweep + nudge), `alerts.py` (create). **The barangay dashboard's queries in `backend/app/api/routes/barangay.py` still need this filter added — that file is the `dashboard` lane's (§15).**

...
- **Family account** (`POST /auth/me/delete`, JWT-authenticated): the `users` row is kept for audit with `deleted_at` set; `is_active` goes False (so `login()` / `get_current_user()` already reject it); every `contacts` pairing is soft-unlinked (`unlinked_by = family`); `device_tokens` rows are deleted; and `username` / `email` / `google_sub` are tombstoned with a `+deletedNNN` suffix so those unique slots free up for a fresh sign-up — no constraint change, deliberately not a partial index.
